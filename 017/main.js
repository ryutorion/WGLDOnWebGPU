...
function frame(timestamp){
    ...
    world = mat4x4.Translation(new vec3(x, y + 1, 0));
    wvp = world.mul(vp);
    device.queue.writeBuffer(uniformBuffers[0], 0, wvp.buffer);

    world = mat4x4.RotationY(rad).mul(mat4x4.Translation(new vec3(1, -1, 0)));
    wvp = world.mul(vp);
    device.queue.writeBuffer(uniformBuffers[1], 0, wvp.buffer);

    world = mat4x4.Scale(new vec3(y + 1, y + 1, 0)).mul(mat4x4.Translation(new vec3(-1, -1, 0)));
    wvp = world.mul(vp);
    device.queue.writeBuffer(uniformBuffers[2], 0, wvp.buffer);
